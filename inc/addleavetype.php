<?php
    ...
    if(isset($_POST['addleave'])){
        ...
        $sql = "SELECT LeaveName FROM leavetype WHERE LeaveName = '".$lname."'";
        $resultset = mysqli_query($con,$sql);
        if(mysqli_num_rows($resultset) > 0){
            header("Location: ../Employee/Leave/AddLeaveType.php?error=nametaken&type=".$emptype."&days=".$noofdays."&desc=".$description);
            exit();
        }else{
            $query = "INSERT INTO leavetype(LeaveName,EmpType,NoofDays,Description) VALUES ('$lname','$emptype','$noofdays','$description')";
